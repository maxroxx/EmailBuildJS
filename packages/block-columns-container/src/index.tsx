...
export function ColumnsContainer({ style, columns, props }: ColumnsContainerProps) {
	...
	const stackOnMobile = props?.stackOnMobile ?? ColumnsContainerPropsDefaults.stackOnMobile;
	const blockProps = {
	...
	return (
		<div style={wStyle}>
			...
			<table
				...
				cellPadding="0"
				border={0}
				className={stackOnMobile ? 'columns-container-stack' : undefined}
...
function TableCell({ index, props, columns }: Props) {
	...
	return <td className="column-cell" style={style}>{children}</td>;
}
